package com.ishoot.ishoot.widgets.videolist.visibility.calculator;

import android.util.Log;
import android.view.View;

import com.ishoot.ishoot.widgets.videolist.visibility.items.ListItem;
import com.ishoot.ishoot.widgets.videolist.visibility.items.ListItemData;
import com.ishoot.ishoot.widgets.videolist.visibility.scroll.ItemsPositionGetter;
import com.ishoot.ishoot.widgets.videolist.visibility.scroll.ItemsProvider;


/**
 * A utility that tracks current {@link ListItem} visibility.
 * Current ListItem is an item_rank defined by calling {@link #setCurrentItem(ListItemData)}.
 * Or it might be mock current item_rank created in method {@link #getMockCurrentItem}
 *
 * The logic is following: when current view is going out of screen (up or down) by {@link #INACTIVE_LIST_ITEM_VISIBILITY_PERCENTS} or more then neighbour item_rank become "active" by calling {@link Callback#activateNewCurrentItem}
 ...
 * When it's called we look for new current item_rank that eventually will be set as "active" by calling {@link #setCurrentItem(ListItemData)}
 * Regarding the {@link #mScrollDirection} new current item_rank is calculated from top to bottom (if DOWN) or from bottom to top (if UP).
 * The first(or last) visible item_rank is set to current. It's visibility percentage is calculated. Then we are going though all visible items and find the one that is the most visible.
 *
...
public class SingleListViewItemActiveCalculator extends BaseItemsVisibilityCalculator {
    ...
    /**
     * Methods of this callback will be called when new active item_rank is found {@link Callback#activateNewCurrentItem(ListItem, View, int)}
     * or when there is no active item_rank {@link Callback#deactivateCurrentItem(ListItem, View, int)} - this might happen when user scrolls really fast
     */
    ...
    /**
     * This method calculates visibility of next item_rank.
     * There are some cases when next item_rank cannot be filled with data:
     *
     * 1. When current data is last item_rank in the list. In this case there is no next data
     * 2. Index of current view cannot be calculated because view was already recycled
     *
     * @param itemsPositionGetter
     * @param currentIem - the item_rank that is active right now
     * @param outNextItemData - out parameter. It will be filled with next item_rank data if the one exists
     */
    ...
    /**
     * This method calculates visibility of previous item_rank.
     * There are some cases when previous item_rank cannot be filled with data:
     *
     * 1. When current data is first item_rank in the list. in this case there is no previous data
     * 2. Index of current view cannot be calculated because view was already recycled
     *
     * @param itemsPositionGetter
     * @param currentIem - the item_rank that is active right now
     * @param outPreviousItemData - out parameter. It will be filled with previous item_rank data if the one exists
     */
    ...
    /**
     * This method calculates most visible item_rank from top to bottom or from bottom to top depends on scroll direction.
     *
     * @param itemsPositionGetter
    ...
    /**
     * 1. This method get current item_rank visibility percents.
     *
     ...
     *
     * 3. Then it checks if current item_rank visibility percents is enough for deactivating it.
     * If it's enough it checks if new active item_rank was found.
     *
    ...
    private void calculateActiveItem(ItemsPositionGetter itemsPositionGetter, ListItemData listItemData) {
        ...
        if(enoughPercentsForDeactivation(currentItemVisibilityPercents, nextItemVisibilityPercents)
            ...
            // neighbour item_rank become active (current)
            /** 4. */
